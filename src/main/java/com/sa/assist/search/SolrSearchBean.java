...
public class SolrSearchBean implements ISearchProvider {
...
//		}
	...
	private void addPostcodeFilter(String searchPC, SolrQuery q, boolean isBedSearch) {
		...
		if (StringUtils.isNotBlank(searchPC) && !searchPC.equalsIgnoreCase("POSTCODE")){
			...
			//StringBuilder filterBuff = new StringBuilder("(type:CMS_* OR (type:vendor AND vendortype:" + VendorType.CAREHOME.getTypeId() + ") OR ((type:vendor OR type:service OR type:product) ");
//			StringBuilder filterBuff = new StringBuilder("(type:CMS_* OR (type:vendor AND vendortype:" + VendorType.CAREHOME.getTypeId() + ") ");
			
			StringBuilder filterBuff = new StringBuilder("(type:CMS_*) ");
			boolean eventsEnabled = false;
			SystemConfig sc =  getSystemConfig();
//			if(sc.getModules().isEventsEnabled()){
//				eventsEnabled = true;
//			}
			...
			if(eventsEnabled){
				filterBuff.append("OR (type:event)");
			}
			filterBuff.append("OR ((type:vendor OR type:service OR type:product)");
		
			
			if (isBedSearch){
				filterBuff = new StringBuilder("(((type:carehome_bed) ");
			}	
...
//				}
	...
	private void addManagedVendorsOnlyFilter(SolrQuery q) {
		q.addFilterQuery("(type:vendor AND vendormanaged:true)");
	}
	
	private void addRegisteredVendorsOnly(SolrQuery q) {
		q.addFilterQuery("(type:vendor AND vendormanaged:false)");
	}
	
	private void addContractedFilter(SolrQuery q) {
		q.addFilterQuery("((type:vendor OR type:service OR type:product) AND contracted:true)");
	}
	
	private void addKitemarkFilter(SolrQuery q) {
		q.addFilterQuery("((type:vendor OR type:service OR type:product) AND kitemark:true)");
	}
	
	@Override
	public SearchResult getSearchResults(String searchText, Integer pageNum, Integer pSize, String sortOption, 
		...
		String relevanceQuery = getRelevenceQuery(searchText);
		q.setQuery(relevanceQuery);
...
//		    	q.addFilterQuery("needtree:" + sbNeedFilter.toString());
		...
		try {
			...
			if (hitLen == 0 && StringUtils.isNotBlank(searchText)) {
				
//				if (getSystemConfig().isSearchDidYouMean()) {
//					List<Suggestion> suggestionList = getSuggesterSearchResults(searchText,1);
//					
...
//			HashMap<String, String> otMap = organisationTypeDAO.getOrganisationTypesAsMap();
			for (int i=0; i < hitLen; i++) {
				...
				StandardProduct p = getResult(hitLen, i, doc, null, geoSearch, context); 
			...
			//result.setSearchPostcode(searchPC);
			
			result.setCurrentPage(pageNum);
			result.setPageSize(pSize);
			int lastPage = (totalResults / pSize) + ((totalResults % pSize) > 0 ? 1 : 0);
			result.setLastPage(lastPage);
			
			int firstNavPage = Math.max(pageNum - navPageCount/2, 1);
			int lastNavPage = Math.min(lastPage, firstNavPage + navPageCount - 1);
			result.setFirstNavPage(firstNavPage);
			result.setLastNavPage(lastNavPage);
			
			int firstResult = ((pageNum-1) * pSize) + 1;
			result.setFirstResult(firstResult);
			result.setLastResult(Math.min(firstResult + pSize - 1, totalResults));		
...
//		result.setBedAvailabilityMap(map);
	...
	private StandardProduct getResult(int hitLen, int i, SolrDocument doc, HashMap<String, String> otMap, boolean geoSearch, String context) {
		...
		if (doc.containsKey("pageUrl")) {
			...
			if (cmsContent == true) {
				...
				p.setPageUrl("/"+ context + (String)doc.getFieldValue("pageUrl"));
			}
		}
		...
		if (doc.containsKey("vendorparentname")) {
			p.setVendorParentName((String)doc.getFieldValue("vendorparentname"));
		}
		
		if (doc.containsKey("vendorparentid")) {
			p.setVendorParentId((Integer)doc.getFieldValue("vendorparentid"));
		}
		...
		if (doc.containsKey("vendoremail")) {
			p.setVendorEmail((String)doc.getFieldValue("vendoremail"));
		}
		
		if (doc.containsKey("vendorwebsite")) {
			p.setVendorWebsite((String)doc.getFieldValue("vendorwebsite"));
		}
		
		if (doc.containsKey("vendortelephone")) {
			p.setVendorTelephone((String)doc.getFieldValue("vendortelephone"));
		}
		...
		if (doc.containsKey("productCount")) {
			p.setProductCount((Integer)doc.getFieldValue("productCount"));
		}
		if (doc.containsKey("serviceCount")) {
			p.setServiceCount((Integer)doc.getFieldValue("serviceCount"));
		}
...
//		}
		...
		if (doc.containsKey("availabledates")) {
			...
			for (Object obj:avDates){
				Date d = (Date)obj;
				p.getAvailabledates().add(d);
			}
		}
		
		if (doc.containsKey("smartsuggest")) {
			p.setSmartSuggest((Boolean)doc.getFieldValue("smartsuggest"));
		}
